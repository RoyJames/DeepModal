...
sphere_grid = bempp.api.shapes.sphere(h=0.01,r = 0.1*3**(0.5))
faces = sphere_grid.elements.T
vertices = sphere_grid.vertices.T
points = vertices[faces].mean(1)

dp0_space_sphere = bempp.api.function_space(sphere_grid, "DP", 0)
points_dirichlet = []
...
for x in points:
    ...
    points_dirichlet.append(my_hankel(1,10)*my_sph_harm(1,1,theta,phi))

export(f'test.msh', grid_function=GridFunction(dp0_space_sphere, coefficients=np.asarray(points_dirichlet)))
